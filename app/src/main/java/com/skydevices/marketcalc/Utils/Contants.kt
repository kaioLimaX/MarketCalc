package com.skydevices.marketcalc.Utils

import androidx.core.content.ContextCompat
import com.skydevices.marketcalc.R
import com.skydevices.marketcalc.Utils.dialogUtil.DialogInfo


object Contants {

    //increment
    const val minValue = 1
    const val maxValue = 99

    const val mensagemTermos = """
Bem-vindo ao nosso aplicativo de soma de mercado.
Ao usar o aplicativo, você concorda com os seguintes termos de uso:

    1. O aplicativo é destinado para organização de listas de compras e cálculo do preço total.

    2. Você é responsável pela precisão das informações inseridas no aplicativo.

    3. Não coletamos informações financeiras pessoais.

    4. Use o aplicativo de acordo com as leis locais e nacionais.

    5. Não somos responsáveis por disputas ou perdas financeiras.
    
Ao clicar em "Aceitar", você concorda com esses termos de uso.
    """


}